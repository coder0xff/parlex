...
context::context(subjob & owner, context_ref const & prior, int documentPosition, match const * fromTransition) :
 owner(owner), prior(prior), current_document_position(documentPosition),
 from_transition(fromTransition != nullptr ? new match(*fromTransition) : nullptr), id (++contextIDCounter),
